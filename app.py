...
def run_migrations():
    ...
    with get_db() as db:
        ...
        if 'products' in existing_tables:
            ...
            # Migration API Products columns
            for col_def in [
                "ALTER TABLE products ADD COLUMN api_product_id INTEGER",
                "ALTER TABLE products ADD COLUMN api_source TEXT",
                "ALTER TABLE products ADD COLUMN api_price INTEGER DEFAULT 0",
                "ALTER TABLE products ADD COLUMN markup_pct INTEGER DEFAULT 5",
                "ALTER TABLE products ADD COLUMN api_last_sync TEXT",
            ]:
                try:
                    cur.execute(col_def)
                    logger.info(f"✅ Migration: {col_def[:60]}")
                except Exception:
                    pass
...
def init_db():
    ...
    with get_db() as db:
        ...
        default_configs = [
            ...
            # Canboso API defaults
            ('canboso_api_key', 'tgb_0fd90a0ac39d44a5cb46910bf63ef8ed6f61e813c92b4e1a'),
            ('canboso_base_url', 'https://canboso.com/api'),
            ('canboso_markup', '5'),
            ('canboso_sync_interval', '10'),
            ('canboso_enabled', '0'),
            ('canboso_auto_sync', '0'),
        ]
...
def ensure_columns():
    ...
    with get_db() as db:
        ...
        try:
            ...
            pass
        try:
            cur.execute("ALTER TABLE products ADD COLUMN api_stock INTEGER DEFAULT 0")
        except:
            pass
        db.commit()
    ...
    with get_db() as db:
        ...
        query = """
            SELECT p.*, c.name as category_name, c.icon as category_icon,
                   CASE
                       WHEN p.api_source IS NOT NULL AND p.api_source != ''
                       THEN COALESCE(p.api_stock, 0)
                       ELSE (SELECT COUNT(*) FROM stocks WHERE product_id = p.id AND is_sold = 0)
                   END as stock_count
            FROM products p
...
# ================= CANBOSO API =================
def get_canboso_api_key() -> str:
    return get_config('canboso_api_key', '').strip()

def get_canboso_base_url() -> str:
    return get_config('canboso_base_url', 'https://canboso.com/api').rstrip('/')

def fetch_canboso_products() -> list:
    """
    GET /api/telegram-buyer/products
    Auth: ?key=<api_key>
    Response: { success, products: [ { _id, product_name, pricing, walletPricing, description, stats } ] }
    """
    try:
        api_key = get_canboso_api_key()
        if not api_key:
            logger.error("Canboso API key chưa được cấu hình!")
            return []

        url = f"{get_canboso_base_url()}/telegram-buyer/products"
        response = requests.get(url, headers={'Accept': 'application/json'},
                                params={'key': api_key}, timeout=15)
        logger.info(f"Canboso products: {response.status_code}")

        if response.status_code == 200:
            data = response.json()
            logger.info(f"Canboso response keys: {list(data.keys()) if isinstance(data, dict) else 'list'}")
            if isinstance(data, dict):
                products = data.get('products', data.get('data', []))
                if isinstance(products, list):
                    logger.info(f"Canboso products count: {len(products)}")
                    return products
            elif isinstance(data, list):
                return data
            logger.warning(f"Canboso unexpected format: {str(data)[:200]}")
        else:
            logger.error(f"Canboso products error: {response.status_code} – {response.text[:300]}")
    except Exception as e:
        logger.error(f"fetch_canboso_products error: {e}")
    return []

def get_canboso_balance() -> dict:
    """
    GET /api/telegram-buyer/balance
    Auth: ?key=<api_key>
    Response: { success, balance, balanceVnd, usdtBalance, walletCurrency }
    """
    try:
        api_key = get_canboso_api_key()
        if not api_key:
            return {'success': False, 'error': 'API key chưa được cấu hình'}

        url = f"{get_canboso_base_url()}/telegram-buyer/balance"
        response = requests.get(url, headers={'Accept': 'application/json'},
                                params={'key': api_key}, timeout=10)
        logger.info(f"Canboso balance: {response.status_code}")

        if response.status_code == 200:
            return response.json()
        else:
            logger.error(f"Canboso balance error: {response.status_code} – {response.text[:200]}")
            return {'success': False, 'error': f"HTTP {response.status_code}: {response.text[:100]}"}
    except Exception as e:
        logger.error(f"get_canboso_balance error: {e}")
        return {'success': False, 'error': str(e)}

def purchase_canboso_product(api_product_id: str, quantity: int = 1,
                              customer_email: str = None, slot_months: int = None) -> dict:
    """
    POST /api/telegram-buyer/purchase
    Auth: key trong request BODY (không dùng query param)
    Body: { key, product_id, quantity, customer_email?, slot_months? }
    Response: { success, orderCode, amount, balance, deliveredAccounts: [{ user, password, verifyEmail, deliveredAt }] }
    """
    try:
        api_key = get_canboso_api_key()
        if not api_key:
        ...
        url = f"{get_canboso_base_url()}/telegram-buyer/purchase"
        payload = {
            'key': api_key,
        ...
        logger.info(f"Canboso purchase: {response.status_code} – product_id={api_product_id}, qty={quantity}")
        if response.status_code == 200:
            result = response.json()
            logger.info(f"Canboso purchase result: orderCode={result.get('orderCode')}, "
                        f"amount={result.get('amount')}, balance={result.get('balance')}, "
                        f"deliveredAccounts={len(result.get('deliveredAccounts', []))}")
            return result
        else:
            logger.error(f"Canboso purchase error: {response.status_code} – {response.text[:500]}")
            return {'success': False, 'error': f"API lỗi {response.status_code}: {response.text[:200]}"}
    except Exception as e:
        logger.error(f"purchase_canboso_product error: {e}")
        return {'success': False, 'error': str(e)}

def format_canboso_delivered_accounts(purchase_result: dict) -> str:
    """
    Chuyển deliveredAccounts từ Canboso purchase response thành chuỗi text cho user.
    Format mỗi account: user | password | verifyEmail
    """
    accounts = purchase_result.get('deliveredAccounts', [])
    if not accounts:
        # Fallback: tìm content ở các key khác
        for key in ('data', 'content', 'items', 'result', 'account'):
            val = purchase_result.get(key)
            if val:
    ...
    for i, acc in enumerate(accounts, 1):
        if isinstance(acc, dict):
            parts = []
            if acc.get('user'):    parts.append(f"User: {acc['user']}")
            if acc.get('password'): parts.append(f"Pass: {acc['password']}")
            if acc.get('verifyEmail'): parts.append(f"Email xác minh: {acc['verifyEmail']}")
            lines.append(f"{'─'*30}\n" + '\n'.join(parts) if parts else str(acc))
        else:
...
def sync_canboso_products() -> dict:
    """
    Đồng bộ sản phẩm từ Canboso API vào DB.
    Tự động tạo/cập nhật category "🌐 API Products" nếu chưa có.
    Canboso dùng _id (string ObjectId) làm product identifier.
    """
    if get_config('canboso_enabled', '0') != '1':
        return {'synced': 0, 'created': 0, 'updated': 0, 'errors': 0, 'message': 'API Products chưa được bật'}

    products = fetch_canboso_products()
    if not products:
        return {'synced': 0, 'created': 0, 'updated': 0, 'errors': 0, 'message': 'Không lấy được sản phẩm từ API – kiểm tra API key và kết nối mạng'}

    global_markup = int(get_config('canboso_markup', '5'))
    ...
    with get_db() as db:
        ...
        # Đảm bảo cột api_product_id là TEXT (có thể là ObjectId string)
        # Đảm bảo category "🌐 API Products" tồn tại
        cur.execute("SELECT id FROM categories WHERE name = '🌐 API Products'")
        cat_row = cur.fetchone()
        if cat_row:
            api_cat_id = cat_row[0]
        else:
            cur.execute(
                "INSERT INTO categories (name, description, icon, is_active, sort_order) VALUES (?, ?, ?, 1, 999)",
                ('🌐 API Products', 'Sản phẩm từ canboso.com', '🌐')
            )
            api_cat_id = cur.lastrowid
            db.commit()
            logger.info(f"✅ Created '🌐 API Products' category (id={api_cat_id})")

        for prod in products:
            try:
                # Canboso dùng _id (MongoDB ObjectId string)
                canboso_id = prod.get('_id') or prod.get('id') or prod.get('product_id')
                if not canboso_id:
                    logger.warning(f"Skipping product with no ID: {prod}")
                    continue

                canboso_id = str(canboso_id)  # Đảm bảo là string

                # Map fields từ Canboso response
                # product_name, pricing (VNĐ), description, stats.available
                prod_name = prod.get('product_name') or prod.get('name') or f'Sản phẩm #{canboso_id[-6:]}'
                prod_desc = prod.get('description') or ''
                # Canboso dùng "pricing" (VNĐ) - không phải walletPricing
                api_price = int(
                    prod.get('pricing') or
                    prod.get('walletPricing') or
                    prod.get('wallet_pricing') or
                    prod.get('price') or
                    0
                )
                markup_pct = global_markup
                sell_price = int(api_price * (1 + markup_pct / 100))

                # Stock available từ stats.available (tồn kho thực tế của shop nguồn)
                api_stock = 0
                stats_data = prod.get('stats') or {}
                if isinstance(stats_data, dict):
                    api_stock = int(stats_data.get('available', 0))

                # Kiểm tra đã tồn tại (so sánh string)
                cur.execute(
                    "SELECT id FROM products WHERE api_product_id = ? AND api_source = 'canboso'",
                    (canboso_id,)
                )
                ...
                if existing:
                    cur.execute("""
                        UPDATE products
                        SET name=?, description=?, api_price=?, markup_pct=?, price=?, api_last_sync=?, api_stock=?
                        WHERE api_product_id=? AND api_source='canboso'
                    """, (prod_name, prod_desc, api_price, markup_pct, sell_price, now_str, api_stock, canboso_id))
                    stats['updated'] += 1
                else:
                    cur.execute("""
                        INSERT INTO products
                        (category_id, name, description, price, is_active, contact_seller,
                         sort_order, api_product_id, api_source, api_price, markup_pct, api_last_sync, api_stock)
                        VALUES (?, ?, ?, ?, 1, 0, 0, ?, 'canboso', ?, ?, ?, ?)
                    """, (api_cat_id, prod_name, prod_desc, sell_price,
                          canboso_id, api_price, markup_pct, now_str, api_stock))
                    stats['created'] += 1

                stats['synced'] += 1
                logger.info(f"  ✅ Synced: [{canboso_id[-8:]}] {prod_name} – {api_price:,}đ → {sell_price:,}đ")

            except Exception as e:
                logger.error(f"sync_canboso_products – error for product {prod}: {e}")
                stats['errors'] += 1

        db.commit()

    logger.info(f"✅ Canboso sync done: {stats}")
    return stats

async def canboso_sync_loop():
    """Vòng lặp tự động sync sản phẩm Canboso định kỳ"""
    global canboso_sync_task
    logger.info("Canboso sync loop started")
    while True:
        try:
            if get_config('canboso_auto_sync', '0') == '1' and get_config('canboso_enabled', '0') == '1':
                result = sync_canboso_products()
                logger.info(f"Canboso auto-sync: {result}")
            interval_min = int(get_config('canboso_sync_interval', '10'))
            await asyncio.sleep(interval_min * 60)
        except asyncio.CancelledError:
            break
        except Exception as e:
            logger.error(f"Canboso sync loop error: {e}")
            await asyncio.sleep(60)
    logger.info("Canboso sync loop stopped")
...
async def api_confirm_order(order_code: str, request: Request, auth: bool = Depends(require_admin)):
    ...
    if is_api_product:
        ...
        if purchase_result:
            product_content = (
                purchase_result.get('data') or purchase_result.get('content') or
                purchase_result.get('product_content') or purchase_result.get('account') or
                purchase_result.get('items') or purchase_result.get('result')
            )
            if isinstance(product_content, list):
                product_content = '\n'.join(str(x) for x in product_content)
            elif isinstance(product_content, dict):
                product_content = json.dumps(product_content, ensure_ascii=False, indent=2)
            elif product_content is not None:
                product_content = str(product_content)
        if product_content:
            send_telegram_sync(
                order['user_id'],
                f"🎉 <b>ĐƠN HÀNG ĐƯỢC XÁC NHẬN!</b>\n\n"
                f"📦 <b>SẢN PHẨM API:</b>\n<code>{product_content}</code>"
            )
            return {"success": True, "delivered": True, "type": "api"}
        else:
            error = purchase_result.get('error', 'Lỗi không xác định') if purchase_result else 'Không nhận được response'
            send_telegram_sync(order['user_id'], f"⚠️ Hết hàng hoặc lỗi API! Admin sẽ liên hệ.\nLỗi: {error}")
            return {"success": True, "delivered": False, "error": error}
...
# ================= CANBOSO ADMIN API =================
class CanbosoConfig(BaseModel):
    api_key: str = ""
    base_url: str = "https://canboso.com/api"
    markup: int = 5
    sync_interval: int = 10
    enabled: bool = False
    auto_sync: bool = False

class CanbosoProductUpdate(BaseModel):
    name: str
    description: str = ""
    category_id: int
    markup_pct: int = 5
...
@app.get("/api/admin/canboso/config")
async def api_get_canboso_config(request: Request, auth: bool = Depends(require_admin)):
    """Lấy cấu hình Canboso API"""
    return {
        "api_key": get_config('canboso_api_key', ''),
        "base_url": get_config('canboso_base_url', 'https://canboso.com/api'),
        "markup": int(get_config('canboso_markup', '5')),
        "sync_interval": int(get_config('canboso_sync_interval', '10')),
        "enabled": get_config('canboso_enabled', '0') == '1',
        "auto_sync": get_config('canboso_auto_sync', '0') == '1',
    }

@app.post("/api/admin/canboso/config")
async def api_save_canboso_config(data: CanbosoConfig, request: Request, auth: bool = Depends(require_admin)):
    """Lưu cấu hình Canboso API"""
    global canboso_sync_task
    try:
        set_config('canboso_api_key', data.api_key.strip())
        set_config('canboso_base_url', data.base_url.strip().rstrip('/'))
        set_config('canboso_markup', str(data.markup))
        set_config('canboso_sync_interval', str(data.sync_interval))
        set_config('canboso_enabled', '1' if data.enabled else '0')
        set_config('canboso_auto_sync', '1' if data.auto_sync else '0')

        # Khởi động/dừng sync loop
        if data.auto_sync and data.enabled:
            if canboso_sync_task is None or canboso_sync_task.done():
                canboso_sync_task = asyncio.create_task(canboso_sync_loop())
        else:
            if canboso_sync_task and not canboso_sync_task.done():
                canboso_sync_task.cancel()
                canboso_sync_task = None

        return {"success": True, "message": "Đã lưu cấu hình Canboso!"}
    except Exception as e:
        logger.error(f"Save Canboso config error: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/admin/canboso/products")
async def api_get_canboso_products(request: Request, auth: bool = Depends(require_admin)):
    """Lấy danh sách sản phẩm API từ DB (đã sync)"""
    with get_db() as db:
        cur = db.cursor()
        cur.execute("""
            SELECT p.*, c.name as category_name
            FROM products p
            LEFT JOIN categories c ON p.category_id = c.id
            WHERE p.api_source = 'canboso'
            ORDER BY p.id DESC
        """)
        return [dict(row) for row in cur.fetchall()]

@app.get("/api/admin/canboso/balance")
async def api_get_canboso_balance(request: Request, auth: bool = Depends(require_admin)):
    """Lấy số dư ví Canboso theo thời gian thực"""
    result = get_canboso_balance()
    return result

@app.post("/api/admin/canboso/sync")
async def api_canboso_sync(request: Request, auth: bool = Depends(require_admin)):
    """Sync sản phẩm từ Canboso ngay lập tức"""
    try:
        result = sync_canboso_products()
        # Coi là lỗi nếu không sync được sản phẩm nào và có message lỗi
        if result.get('synced', 0) == 0 and result.get('message'):
            return {"success": False, **result}
        return {"success": True, **result}
    except Exception as e:
        logger.error(f"Canboso manual sync error: {e}")
        return {"success": False, "message": str(e)}

@app.put("/api/admin/canboso/products/{product_id}")
async def api_update_canboso_product(
    product_id: int,
    data: CanbosoProductUpdate,
    request: Request,
    auth: bool = Depends(require_admin)
):
    """Cập nhật thông tin sản phẩm API (tên, markup, kích hoạt)"""
    try:
        with get_db() as db:
            cur = db.cursor()
            # Tính lại giá bán dựa trên markup mới
            cur.execute("SELECT api_price FROM products WHERE id=? AND api_source='canboso'", (product_id,))
            row = cur.fetchone()
            if not row:
                return {"success": False, "message": "Sản phẩm không tồn tại"}
            api_price = row[0] or 0
            sell_price = int(api_price * (1 + data.markup_pct / 100))
            cur.execute("""
                UPDATE products
                SET name=?, description=?, category_id=?, markup_pct=?, price=?, is_active=?
                WHERE id=? AND api_source='canboso'
            """, (data.name, data.description, data.category_id, data.markup_pct, sell_price,
                  1 if data.is_active else 0, product_id))
            db.commit()
        return {"success": True, "message": "Đã cập nhật sản phẩm!"}
    except Exception as e:
        logger.error(f"Update Canboso product error: {e}")
        return {"success": False, "message": str(e)}

@app.get("/api/admin/canboso/fetch")
async def api_canboso_fetch_raw(request: Request, auth: bool = Depends(require_admin)):
    """Lấy danh sách sản phẩm thô từ Canboso API (để xem trước)"""
    try:
        products = fetch_canboso_products()
...
@app.get("/api/admin/canboso/test-connection")
async def api_canboso_test_connection(request: Request, auth: bool = Depends(require_admin)):
    """Test kết nối API Canboso – trả về raw response để debug"""
    try:
        api_key = get_canboso_api_key()
        base_url = get_canboso_base_url()
        url = f"{base_url}/telegram-buyer/products"
        resp = requests.get(url, headers={'Accept': 'application/json'}, params={'key': api_key}, timeout=15)
        try:
            body = resp.json()
        except Exception:
            body = resp.text[:500]
        return {
            ...
            "url": url,
            "api_key_preview": api_key[:12] + '...' if api_key else '(empty)',
            "response": body,
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def on_startup():
    ...
    # Bật Canboso sync nếu được cấu hình
    if get_config('canboso_auto_sync', '0') == '1' and get_config('canboso_enabled', '0') == '1':
        global canboso_sync_task
        canboso_sync_task = asyncio.create_task(canboso_sync_loop())
        logger.info("Canboso auto-sync: STARTED")
